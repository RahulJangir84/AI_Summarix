    ...
    <>
      ...
      <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end" style={{ fontFamily: "'DM Sans', sans-serif" }}>
        ...
        <AnimatePresence>
          {isOpen && (
            <motion.div
              ...
              className="mb-4 relative flex flex-col rounded-2xl overflow-hidden shadow-2xl shadow-indigo-200/60 border border-slate-100"
            >
              {/* Background */}
              <div className="absolute inset-0 bg-slate-50" />
              <div className="absolute inset-0 bg-gradient-to-br from-indigo-50/80 via-white to-white" />
              ...
              {/* Header */}
              <div className="relative flex items-center justify-between px-5 pt-8 pb-2 bg-white border-b border-slate-100 shadow-sm">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-slate-800 text-[13px] font-semibold leading-none mb-0.5">Document Assistant</p>
              ...
              <div className="relative flex-1 overflow-y-auto p-4 flex flex-col gap-3 chat-scroll">
                {messages.length === 0 ? (
                  <div className="h-full flex flex-col items-center justify-center gap-3 text-center px-8">
                    ...
                    <div>
                      <p className="text-slate-700 text-[17px] font-semibold mb-1">Ask about your document</p>
                    ...
                    <div className="flex flex-wrap gap-2 justify-center mt-1">
                      {["Summarize this", "Key takeaways?", "Main topics?"].map((prompt) => (
                        <button
                          ...
                          className="text-[14px] px-3 py-1.5 rounded-lg bg-white border border-indigo-100 text-indigo-500 font-medium hover:bg-indigo-50 hover:border-indigo-300 transition-all shadow-sm"
                        >
                          {prompt}
                        </button>
                      ))}
                ...
                {status === "submitted" && messages[messages.length - 1]?.role === "user" && (
                  ...
                  >
                    <div className="w-7 h-7 rounded-full bg-white border border-indigo-100 flex items-center justify-center shrink-0 shadow-sm">
              ...
              {/* Input */}
              <div className="relative px-4 py-3 bg-white border-t border-slate-100">
                <form
                  onSubmit={(e) => {
                    e.preventDefault();
                  ...
                  className="flex items-center gap-2 bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 focus-within:border-indigo-400 focus-within:bg-white focus-within:shadow-sm focus-within:shadow-indigo-100 transition-all"
                >
                  <input
                    type="text"
                    value={input}
                    ...
                    className="flex-1 bg-transparent text-[13px] text-slate-700 placeholder:text-slate-400 focus:outline-none disabled:opacity-50"
                  />
                  <button
                    type="submit"
                    disabled={isLoading || !input.trim()}
                    className="w-7 h-7 rounded-lg bg-indigo-600 hover:bg-indigo-700 disabled:opacity-30 disabled:hover:bg-indigo-600 flex items-center justify-center transition-all shrink-0 shadow-sm shadow-indigo-200"
                  >
        ...
        <motion.button
          ...
          className="relative w-14 h-14 rounded-2xl bg-indigo-600 hover:bg-indigo-700 shadow-xl shadow-indigo-300/50 flex items-center justify-center transition-colors"
        >
